...
// 1チーム分の form / 順位由来のファクトを集める。
void collect_team_facts(const TeamCtx& c, bool for_home, const FactThresholds& th,
                        std::vector<Fact>& out) {
  if (!c.known || !c.now) return;
  const std::string& form = c.now->form;

  // 開幕からの全勝。form 長 == 消化数 かつ 全て W (§4.1)。
...
Fact compute_fact(const Match& m, const StandingsPool& pool,
  ...
  // 勝者側の form ファクトを優先する。負けたチームの連勝は表示しても意味がない。
  if (result >= 0) collect_team_facts(home, true, th, cands);
  if (result <= 0) collect_team_facts(away, false, th, cands);
