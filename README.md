...
### 新着は「結果が届いた時刻」から20時間だけ出す

**1日以上前の結果は出さず、新着が無ければ前回の画面を維持します。**
「1日」はキックオフからではなく、**端末が初めてその試合の完了を確認した時刻**
から数えます（`/cache/seen_fixtures.json`、`src/core/freshness.cpp`）。

以前は「直近72時間・競技会の優先度順」で並べていたため、CL の翌々日まで
CL が画面を占有し、前夜のプレミアが全部 `+N more` に押し出されていました。

キックオフ基準にしない理由は、欧州の遅い試合が 06:00〜07:00 JST 頃に終わり、
API への反映がその後になるためです。07:00 の起床に間に合わなかった試合は、
翌朝の起床時点でキックオフから27時間経っており、キックオフ基準の24時間では
**一度も表示されません。**（実例: 9/13 04:00 JST キックオフの
`Sunderland 0-2 Arsenal` は反映が 07:58 JST だった）

| 設定 | 既定 | 意味 |
|---|---|---|
| `fresh_hours` | 20 | 初めて完了を確認してからこの時間内なら新着 |
| `display_window_hours` | 72 | キックオフがこれより古い試合は出さない |

- 24時間ではなく20時間なのは、毎朝07:00の起床で前日の分がちょうど24時間前後になり、
  取得にかかる数十秒の差で出たり消えたりするため
- **履歴が無いとき**（初回起動・SD 差し替え・ファイル破損・旧形式）は、その回だけ
  「結果はキックオフの2時間後に届いていた」とみなします。45日分すべてが
  「今初めて見た」になり、数日前の試合が新着に出るのを防ぐためです。
  キックオフの上限（72時間）はその安全網です
- 旧ファームの `seen_fixtures.json`（id 一覧）は**信用せず捨てます**。旧ファームは
  画面に入りきらず `+N more` に回した試合まで記録していたので、それを「表示済み」と
  みなすと、一度も表示されていない試合が既読になります（2026-09-13 に実機で発生）
- 新着が無い日は固定エリアに `No new results 09/14 07:00` と出し、止まっているのか
  新着が無いのかを区別できるようにしています。順位表のリクエストも飛びません
- 前回の画面も無い場合（初回起動など）だけ、72時間以内の結果を出します

並び順は従来どおり**競技会の優先度順**（CL > PL > BL1 > PD）です。週末に
新着が画面に入りきらない場合は、優先度の低いリーグから `+N more` に回ります。
